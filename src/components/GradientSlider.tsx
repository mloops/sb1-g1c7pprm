...
export const GradientSlider: React.FC<GradientSliderProps> = ({
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      ...
      {colorLabel && colorClass && (
        <div className="flex justify-center mb-2">
          <div className={`px-3 py-1.5 rounded text-sm font-medium ${colorClass}`}>
            {colorLabel}
          </div>
        </div>
      )}
      ...
      <div className="flex items-center gap-4">
        <div className="flex-1 relative">
          <input
            ...
            max={max}
            step={step}
            className="w-full h-2 gradient-slider rounded-lg appearance-none cursor-pointer"
